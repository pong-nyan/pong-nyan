...
export const socketEmitGameStartEvent = (gameStatus: GameStatus) => {
  ...
  socket.gameNamespace.emit('game-start', {
    gameStatus,
  });
};
...
export const socketEmitGameKeyEvent = (playerNumber: PlayerNumber, opponentId: string, message: KeyEventMessage, step: number = 0, velocity: number = 0) =>
  socket.gameNamespace.emit('game-keyEvent', {
    playerNumber,
    opponentId,
    message,
    step,
...
export const socketOnGameKeyEvent = (engine: Engine | undefined) => {
  socket.gameNamespace.on('game-keyEvent', ({opponentNumber, message, step, velocity}
...
export const socketEmitGameBallEvent = (position: Matter.Vector, velocity: Matter.Vector) =>
  socket.gameNamespace.emit('game-ball', {
    position,
    velocity,
  });
...
export const socketOnGameBallEvent = (engine: Engine | undefined) =>
  socket.gameNamespace.on('game-ball', ({ position, velocity }: { position: Matter.Vector, velocity: Matter.Vector }) => {
...
export const socketEmitGameScoreEvent = (playerNumber: PlayerNumber, score: { p1: Score, p2: Score}) => {
  socket.gameNamespace.emit('game-score', { 
    playerNumber,
    score,
  });
};
...
export const socketOnGameScoreEvent = (sceneSize: CanvasSize, engine: Engine | undefined, runner: Runner | undefined, setScore: Dispatch<SetStateAction<{p1: Score, p2: Score}>>) => {
  socket.gameNamespace.on('game-score', ( { realScore, winnerNickname } : { realScore: Score, winnerNickname: string }) => {
...
export const socketOnGameDisconnectEvent = (sceneSize: CanvasSize, engine: Engine | undefined, runner: Runner | undefined, setScore: Dispatch<SetStateAction<{p1: Score, p2: Score}>>, setGameStatus: Dispatch<SetStateAction<GameStatus>>) => {
  socket.gameNamespace.on('game-disconnect', ( { disconnectNickname, gameInfo } : { disconnectNickname: string, gameInfo: GameInfo } ) => {
    if (!engine || !engine.world || !runner) return;
    Runner.stop(runner);
...
export const socketOnGameStartEvent = (setGameStatus: Dispatch<SetStateAction<GameStatus>>, setPlayerNumber: Dispatch<SetStateAction<PlayerNumber>>, setOpponentId: Dispatch<SetStateAction<SocketId>>) => {
  socket.gameNamespace.on('game-start', ({ player1Id, player2Id, gameStatus }: {roomName: RoomName, player1Id: string, player2Id: string, gameStatus: GameStatus}) => {
    if (socket.gameNamespace.id === player1Id) { 
      setPlayerNumber('player1');
      setOpponentId(player2Id);
    }
    else if (socket.gameNamespace.id == player2Id){
...
export const socketOnGameLoadingEvent = (setLoading: Dispatch<SetStateAction<boolean>>) => {
  socket.gameNamespace.on('game-loading', () => {
    setLoading(true);
  });
};
...
export const socketOnGameEnd = (
  ...
  socket.gameNamespace.on('game-end', ({ winnerNickname, gameInfo } : { winnerNickname: Nickname, gameInfo: GameInfo}) => {
...
export const socketOffGameAllEvent = () => {
  socket.gameNamespace.off('game-keyEvent');
  socket.gameNamespace.off('game-ball');
  socket.gameNamespace.off('game-score');
  socket.gameNamespace.off('game-disconnect');
  socket.gameNamespace.off('game-start');
  socket.gameNamespace.off('game-loading');
  socket.gameNamespace.off('game-end');
};
